...
### 3.1 功能需求
  ...
  H5 与后台共用同一套用户表与 JWT 鉴权；当前实现中，H5 侧发布、承接、状态更新等关键写操作仅允许普通用户角色（USER）执行，管理员与超级管理员主要在后台完成运营与治理，以降低角色混用风险。
  登录账号支持「用户名 / 手机号 / 邮箱 + 密码」三种形式；接口返回 401 时 H5 统一清理本地 token 并跳转登录页，支持 redirect 回跳至原访问路径。
  ...
  业务规则上禁止「发布人承接自己发布的订单」；Bug 广场（可接单列表）默认仅展示待承接（PENDING）数据，避免已承接订单继续占用可接单池。
  

  Bug 列表筛选模块具有多条件筛选、关键词搜索、列表排序、详情查看。
  支持按 Bug 状态、时效状态（正常、即将超期、已超期）等条件筛选，结合关键词搜索与分页，用户可快速定位目标 Bug 并查看详情。
  用户端列表在工程实现上采用移动端滑动触底分页（增量加载）；管理端 PC 列表采用表格分页组件，并与后端 `page` / `pageSize` / `list` / `total` 契约对齐。
  

  后台管理与人工介入模块具有时效监控功能、人工提醒功能、人工介入功能、Bug 数量管理、用户管理功能、运营日志记录。
  管理端（fe-admin）提供时效规则配置（按 Bug 状态配置预警时长与超期时长）、定时时效监控（将 Bug 标记为即将超期或已超期）及超期/预警列表查看，为人工提醒与介入提供数据支撑；支持超级管理员对订单进行人工介入（含运营备注），所有人工介入及关键操作均记录至操作日志，便于追溯；同时提供 Bug/订单列表管理、数量统计与用户治理等功能。
  用户治理按角色分级：普通管理员可在限定范围内维护普通用户启用/禁用状态；新建用户、编辑资料、软删除、分配角色、删除任意订单（软删除）等能力保留给超级管理员，并在接口层通过 `@Roles(SUPER_ADMIN)` 与列表数据过滤（不暴露超级管理员账号）形成双保险。
...
### 3.2 非功能需求
  ...
  接口防刷与风控：登录与业务写接口在后端落地多维限流（如登录侧 IP + 设备 + 账号 + 接口全局，业务侧 userId + 设备 + 接口全局），超限返回 HTTP 429；前端请求自动携带 `x-device-id` 请求头以支持设备维度统计与限流；并可配合 Nginx 网关层限流模板形成「网关 + 应用」双层防护。
...
### 5.1 用户模块
  ...
  核心功能包括用户注册、登录（支持用户名/手机号/邮箱 + 密码）、个人信息查询与修改、权限与角色分配。
  该模块与所有业务模块关联：发布/承接 Bug、管理员人工介入等均依赖用户身份校验；用户表存储 contact_info 等字段，为社群对接提供基础信息。
  后台用户列表接口对超级管理员账号采取「列表不暴露、按角色筛选超级管理员时结果为空、按 ID 查看超级管理员无权限」策略，避免治理账号被误操作或枚举；普通管理员仅可对普通用户（USER）调整启用/禁用状态，且禁止修改自身状态；超级管理员保留完整的用户创建、编辑、角色分配与软删除能力。
...
### 5.2 Bug 发布与承接模块
  ...
  承接环节后端校验「订单须为 PENDING」「承接人不得为发布人」；用户端广场列表默认仅展示可接单（PENDING），与承接规则一致。发布人仅可删除本人发布且尚未被承接的需求；超级管理员可在后台对恶意等内容执行订单软删除并留痕。
...
### 5.4 后台管理模块
  ...
  为人工运营提供工具，仅对管理员开放。
  核心功能包括：时效监控（可配置时效规则、定时任务自动标记即将超期/已超期、超期/预警列表查询）；人工介入（超级管理员可修改订单状态并填写运营备注，操作记入 operation_log）；订单与 Bug 管理（列表、详情、统计）；用户治理（列表筛选、普通管理员对 USER 启用/禁用、超级管理员新建/编辑/分配角色/软删除等）；运营日志（按操作人、类型、时间筛选与查看，仅超级管理员）；时效规则配置（规则增删改查与启用/禁用，与前端路由一致时仅超级管理员可进入配置页）。
  该模块依赖用户、Bug、运营日志与时效规则等数据，是“人工运营 + 社群对接”模式的核心保障。
  前端侧通过路由元信息 `roles` 区分菜单与按钮：文件名带 `.admin` 的界面截图表示普通管理员（ADMIN）可见范围；带 `.super` 的截图表示仅超级管理员（SUPER_ADMIN）可见的扩展菜单或操作；无后缀的截图表示两种管理员视角下界面一致。
...
### 7.1 前端开发环境与工程结构
  ...
  项目初始化：在根目录 `pnpm install` 安装 workspace 依赖；可选用根目录脚本 `pnpm dev:all` 一键拉起数据库（Docker）、构建 shared 并并行启动后端与双端前端（亦提供 `pnpm dev:backend` 仅启后端）；各前端包内配置 Vue Router（用户端/管理端路由分离）、路由守卫（登录与角色校验）；配置 Vite 的 server.proxy 解决开发阶段跨域；封装 Axios 实例，请求拦截器注入 `Authorization: Bearer <token>` 与 `x-device-id`（用于后端设备维度限流），响应拦截器统一处理 401/403/500 并提示。
...
### 7.3 用户端：用户与 Bug 发布承接
  ...
  注册与登录：表单使用 v-model 绑定 RegisterDto/LoginDto 对应字段，校验规则（如手机号正则、密码长度、必填）可用 Vue 或 element-plus 的 form 校验；注册请求 POST /api/user/register，登录请求 POST /api/auth/login，登录成功后保存 token 与用户信息到 userStore 并跳转首页。登录账号输入支持用户名、手机号或邮箱；演示环境可采用 H5 用户 `user_demo / user123`、后台超级管理员 `super_admin / admin123` 等种子账号（以实际种子数据为准）。
  Bug 发布：表单含标题、技术栈（输入或选择）、问题描述（wangEditor 富文本）、预期效果、可选社群信息；提交前前端校验必填与长度；POST /api/bug 传 CreateBugDto，成功后跳转详情或列表。
  Bug 承接：用户在 Bug 广场进入订单详情（如图 7-4-3），在 status 为待解决且当前用户非发布人时展示“承接”按钮；POST /api/bug/:id/take 成功后更新本地状态并弹出社群对接指引（展示 community_info、发布人 contact_info）。「我发布的问题」详情（图 7-3-2）侧重发布人侧状态与操作，与广场承接入口形成互补。
  状态更新：发布人/承接人可选择“沟通中”“已解决”，POST /api/bug/:id/status 传 UpdateStatusDto（status、可选 operationNote），成功后无刷新更新详情。
  个人中心：H5「我的」页拆分为「我的需求」「我发布的」等二级入口，列表采用滑动触底分页；空列表统一使用占位图提示，提升移动端信息层次。发布侧「我发布的」列表见图 7-3-6；承接侧「我的任务 / 我承接的」列表见图 7-3-7，单条任务详情见图 7-3-8。
  个人 Bug 记录：调用 GET /api/order/my，传参 type=published 或 type=taken 分别获取“我发布的”“我承接的”列表（分页）。

![图7-3-1 用户端「发布需求 / 新建我的问题」页面截图](./figures/fe-h5/my-question-new.png)

![图7-3-2 用户端「我发布的问题」详情页截图](./figures/fe-h5/my-question-detail.png)

![图7-3-3 用户端登录页面截图](./figures/fe-h5/user-login.png)

![图7-3-4 用户端注册页面截图](./figures/fe-h5/user-register.png)

![图7-3-5 用户端「我的」入口页面截图](./figures/fe-h5/mine.png)

![图7-3-6 用户端「我发布的问题」列表页面截图（滑动分页）](./figures/fe-h5/my-question-list.png)

![图7-3-7 用户端「我的任务 / 我承接的」列表页面截图（滑动分页）](./figures/fe-h5/my-task-list.png)

![图7-3-8 用户端「我的任务」详情页面截图（承接人侧查看与状态操作）](./figures/fe-h5/my-task-detail.png)
  

### 7.4 用户端：Bug 列表与筛选

  列表页：筛选区（技术栈、状态、时效状态、关键词）；数据区采用移动端列表 + 滑动触底加载更多（由 `@bug/shared` 分页工具与列表底部组件统一驱动），替代传统「上一页/下一页」按钮，以适配 H5 连续浏览习惯。
  筛选变化或触底时带参数请求 GET /api/bug（查询参数 techStack、status、timeStatus、keyword、page、pageSize）。
  ...
  前端将返回数据增量合并至列表，并根据 total 与当前已加载条数判断是否「已经到底」；点击条目进入 /bug/:id 查看详情，详情请求 GET /api/bug/:id 获取完整信息与发布人/承接人，富文本用 v-html 或富文本组件回显。

![图7-4-1 用户端 Bug 广场列表与筛选页面截图](./figures/fe-h5/market.png)

![图7-4-2 用户端 Bug 广场按状态筛选效果截图](./figures/fe-h5/market-status.png)

![图7-4-3 用户端 Bug 广场订单详情页截图（浏览与承接等操作）](./figures/fe-h5/market-detail.png)
  

### 7.5 管理端实现

  时效监控与订单列表：管理端订单列表接口 GET /api/admin/orders，支持 status、timeStatus、keyword、分页；GET /api/admin/overdue-bugs 可获取超期/预警 Bug 列表（与部分列表接口一并支持 `page` / `pageSize` 分页返回）。
  列表展示 Bug 标题、发布人、承接人、状态、时效状态、发布时间等，超期项可高亮；定时刷新（如每 5 分钟）或手动刷新以与后端定时任务同步。PC 端表格分页默认每页条数与 H5 分离配置，由管理端统一分页组件承载。
  ...
  用户管理：GET /api/admin/users 分页与筛选；普通管理员仅可对普通用户（USER）调整启用/禁用；新建用户、编辑资料、分配角色、软删除等由超级管理员接口承担，前端按钮与路由元信息一致收敛。
...
**图 7-5-1 管理端布局（普通管理员 ADMIN，侧栏菜单为运营可见范围）**

![图7-5-1 管理端布局（.admin）](./figures/fe-admin/layout.admin.png)

**图 7-5-2 管理端布局（超级管理员 SUPER_ADMIN，含治理类扩展菜单）**

![图7-5-2 管理端布局（.super）](./figures/fe-admin/layout.super.png)

**图 7-5-3 用户管理页（普通管理员视角：操作列以普通用户状态维护为主，列表不展示超级管理员账号）**

![图7-5-3 用户管理（.admin）](./figures/fe-admin/user.admin.png)

**图 7-5-4 用户管理页（超级管理员视角：含编辑、分配角色、删除等完整治理能力）**

![图7-5-4 用户管理（.super）](./figures/fe-admin/user.super.png)
...
**图 7-2 管理端核心页面截图组（订单列表与详情、时效规则与监控、操作日志；无 `.admin`/`.super` 后缀表示管理员与超级管理员在该类页面上界面一致）**

![图7-2-1 管理端订单列表页面截图](./figures/fe-admin/order.png)

![图7-2-2 管理端订单详情页面截图](./figures/fe-admin/order-detail.png)

![图7-2-3 管理端时效规则配置页面截图](./figures/fe-admin/expired-rule.png)

![图7-2-4 管理端时效监控/预警列表页面截图](./figures/fe-admin/expired-time.png)

![图7-2-5 管理端操作日志页面截图](./figures/fe-admin/operate-log.png)
...
### 8.3 用户模块与认证
  ...
  DTO 设计：RegisterDto 包含 username、phone、email、password、contactInfo（可选）、intro（可选），校验如 @Matches(/^1[3-9]\d{9}$/) 手机号、@Matches(/^[\w.-]+@[\w.-]+\.\w+$/) 邮箱、@MinLength(6) 密码；LoginDto 包含 account（用户名、手机号或邮箱均可）、password。
  ...
  UserService.findByAccount(account) 支持按用户名、手机号或邮箱解析登录账号；validatePassword(plain, hash) 使用 bcrypt.compare。
...
### 8.4 Bug 模块与列表查询
  ...
  BugService.take(bugId, takerId) 校验 bug 存在且 status===PENDING，并禁止承接人等于发布人（防止自撮合）；通过后更新 takerId、status=TAKEN、takeTime，写 BUG_TAKE 日志。
...
### 8.5 后台管理模块
  ...
  getTimeRules 与列表类接口支持 `page` / `pageSize` 分页契约；用户治理方面，`getUserList` 永远不返回 `role === SUPER_ADMIN` 的用户记录，按「超级管理员」筛选时结果为空，关键词检索路径同样排除超级管理员；普通管理员调用 `GET /api/admin/users/:id` 若目标为超级管理员则拒绝。`updateUserStatus` 允许 ADMIN 与 SUPER_ADMIN 调用，但对普通管理员增加约束：仅可修改 `role === USER` 的用户状态，且禁止修改自身状态；新建用户、编辑用户资料、软删除、分配角色等接口保持 `@Roles(SUPER_ADMIN)`。超级管理员可删除任意订单（软删除）用于恶意内容治理，并写入日志。
...
| F008 | 后台管理 | 时效规则配置 | 超级管理员登录，新增规则：关联状态已承接（或沟通中），提醒时长 48 小时，超期时长 72 小时，启用规则 | 规则创建成功，时效规则表记录完整，定时任务（每 5 分钟）按新规则执行并更新 Bug 时效状态 |
...
| R005 | 禁止自承接 | 用户 A 登录，进入自己发布的待承接 Bug 详情页，尝试承接 | 承接失败，接口返回业务错误提示 |
| R006 | 用户列表不暴露超级管理员 | 任意管理员登录后台，打开用户列表或按关键词搜索 | 列表中不出现超级管理员账号；按「超级管理员」角色筛选结果为空 |
| R007 | 普通管理员不可改其他管理员状态 | 普通管理员登录，尝试对另一管理员账号改启用/禁用 | 操作失败或无对应按钮；接口返回无权限 |
| R008 | 登录/写接口限流 | 对登录或写接口短时间高频请求 | 超出阈值返回 HTTP 429，正常流量不受影响 |
...
### 9.9 迭代功能补充验证（安全与体验优化）
  ...
  （5）分页体验与复用优化：在共享包 @bug/shared 中抽离通用分页状态与滚动触底判定工具（createPagerState、patchPagerFromPayload、hasMoreByPayload、shouldTriggerScrollLoad 等），用户端（fe-h5）将 Bug 广场与我的订单列表升级为移动端滑动分页（触底增量加载）；管理端（fe-admin）在订单列表、操作日志、用户管理、时效监控与规则列表等页面统一分页状态处理方式，并与后端 `GET /api/admin/overdue-bugs`、`GET /api/admin/time-rules` 等分页返回对齐。该优化在不改变后端分页接口契约（page/pageSize/list/total）的前提下，提升了移动端连续浏览体验，并降低了前端多列表页面分页逻辑重复度。

  （6）用户管理界面与后端策略一致：管理端 `UserList` 对普通管理员仅保留「状态」类操作，且可点击对象限于非本人的普通用户（本人所在行状态按钮置灰）；超级管理员保留编辑、分配角色、删除等完整操作列，与 `@Roles(SUPER_ADMIN)` 及 `AdminService.updateUserStatus` 的双保险一致。
