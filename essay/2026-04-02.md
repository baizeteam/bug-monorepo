...
### 7.3 用户端：用户与 Bug 发布承接
  ...
  个人中心：H5「我的」页拆分为「我的需求」「我发布的」等二级入口，列表采用滑动触底分页；空列表统一使用占位图提示，提升移动端信息层次。
  个人 Bug 记录：调用 GET /api/order/my，传参 type=published 或 type=taken 分别获取“我发布的”“我承接的”列表（分页）。

![图7-3-1 用户端 Bug 发布页面截图](./figures/fe-h5/push-new.png)

![图7-3-2 用户端 Bug 详情与承接页面截图](./figures/fe-h5/push-detail.png)
...
![图7-3-6 用户端「我的订单」列表页面截图（滑动分页）](./figures/fe-h5/mine-order.png)
